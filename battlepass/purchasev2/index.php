        ...
        if (json_last_error() == 0) {
            ...
            if (file_exists("../../user/db/_$deviceid.json")) {
                ...
                if (md5("thisiskey_lol_".$deviceid) == $player_body->User->Token) {
                    if (json_last_error() == 0) {
                        if ($player_body->User->Balances[3]->Amount >= 1200) {
                            $isPurchase = $player_body->User->BattlePass->HasPurchased;
                            if ($isPurchase == false) {
                                $player_body->User->BattlePass->HasPurchased = true;
                                $player_body->User->Balances[3]->Amount -= 1200;
