            ...
            for ($x = 0; $x < count($users); $x++) {
                ...
                if ($country != "") {
                    if ($player_body->User->Country == $country) {
                        $arr_data = array("Username" => $player_body->User->Username, "Crowns" => $player_body->User->Crowns, "Country" => $country);
                    }             
                }
                else {
                    $arr_data = array("Username" => $player_body->User->Username, "Crowns" => $player_body->User->Crowns, "Country" => $player_body->User->Country);
                }
                array_push($users_list, $arr_data);
            }
            $crown = array_column($users_list, 'Crowns');
