            ...
            if (file_exists("../../../../user/db/_$deviceid.json")) {
                ...
                if (md5("thisiskey_lol_".$deviceid) == $player_body->User->Token) {
                    if (json_last_error() == 0) {
                        ...
                        if ($player_body->User->Balances[11]->Amount >= 55 * $total_spin) {
                            $player_body_no_save = json_decode(file_get_contents("../../../../user/db/_$deviceid.json"));
                            random_common($total_spin, $player_body, $player_body_no_save);
                            $player_body->User->Balances[11]->Amount -= 55 * $total_spin;
                            $player_body_no_save->User->Balances[11]->Amount -= 55 * $total_spin;
